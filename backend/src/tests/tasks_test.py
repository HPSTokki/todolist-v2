...
from src.dtos.todolist_dto import InsertTask, ListReadTask, UpdateTask
...
def test_get_all_task_raw(session):
    service = UserService(session)
    ...
    result = service.get_all_task_raw()
    assert len(result) == 2
...
def test_get_all_task(session):
    ...
    assert isinstance(result, ListReadTask)
    ...
    for id, task in enumerate(result.tasks):
        assert task.id == expected_id[id]
        assert task.title == expected_title[id]
        assert task.description == expected_description[id]
...
def test_delete_task_by_id(session):
    ...
    assert result is not None
    deleted_task = service.get_all_task_raw()
    assert all(task.id != 1 for task in deleted_task)

def test_delete_task_by_name(session):
    service = UserService(session) # TODO FIX THIS LATER
